...
void sendfeed_result(UINT8 feednum)
{
    time_t time_ = 0;
    struct tm tm_;
    PROTOCOL_FEED_RESULT_ST feedResult;
    memset(&feedResult, 0, sizeof(feedResult));
    ...
    feedResult.uFeedNum = feednum;
    feedResult.uYears   = tm_.tm_year - YEAR_START;   /* 0x00标识2021年 */
    feedResult.uMon     = tm_.tm_mon;
    feedResult.uDay     = tm_.tm_mday;
    feedResult.uHour    = tm_.tm_hour;
    feedResult.uMin     = tm_.tm_min;
    feedResult.uSec     = tm_.tm_sec;
    /* 喂食结果上报 */
    sendmessage(sockfd, FEEDER_TO_APP, ID_FEED_RESAULT_REPORT, sizeof(feedResult), (UINT8 *)&feedResult);
    return ;
}
/* 电机控制 */
static void motor_on(INT32 fd, const UINT8 direction, UINT8 feed_num)
{
	...
	INT32 round = MOTOR_ROUND * feed_num; /*5V的是4096个节拍为一圈，12V的暂时不确定  一份半圈*/

	INT32 speed = 0;
	DEBUG("slow %d\n", Get_MotorfeedSpeed());
	if (Get_MotorfeedSpeed() == HIGH)
    {
		speed = MOTOR_SPEED_HIGH;  //快速喂食
    }
	else
    {
		speed = MOTOR_SPEED_LOW;  //慢速喂食
    }
	if (direction == 'R')
		step = -1;
	else
...
static void * client_motor_handle_proc (void * arg)
{
    ...
    DEBUG("%s Init is OK", MOTOR_DEVICE);
